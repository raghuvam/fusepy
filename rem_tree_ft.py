...
class ReliableLayer:
    def __init__(self,urls):
        ...
        self.meta_urls = urls[0]
        
        self.data_urls = urls[1:]

        self.meta_hdl = xmlrpclib.ServerProxy(urls[0])
        self.data_hdls = []
        for url in urls[1:]:
            self.data_hdls.append(xmlrpclib.ServerProxy(url))
...
class FileNode:
    ...
    def put(self,key,value):
        # Call reliable put
        ft_obj= ReliableLayer(urls)
        ft_obj.reliable_put(self.path,key,value)

    def get(self,key):
        # Call realiable get
        ft_obj= ReliableLayer(urls)
        ft_obj.reliable_get(self.path,key)
